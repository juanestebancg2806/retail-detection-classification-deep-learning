...
import os
#file to be replaced in the IQA notebook
...
avg_score = 0.0
filename = sys.argv[2]
total_images = [f for f in os.listdir(sys.argv[1]) if f.endswith('.jpg')]
scores = []
names = []
total_images.sort()
if(len(sys.argv) == 4): total_images = total_images[:int(sys.argv[3])]
for image_name in total_images:
  qualityscore = max(test_measure_BRISQUE(sys.argv[1]+"/"+image_name),0)
  avg_score += qualityscore
  scores.append((image_name,qualityscore))
  print("Score of the {0} given image: {1} ".format(image_name,qualityscore))

scores.sort(key = lambda x: x[1])
names = [scores[i][0] for i in range(len(scores))]
scores = [scores[i][1] for i in range(len(scores))]
scores = np.array(scores,dtype=float)
Q1,Q2,Q3 = np.quantile(scores,.25,interpolation='midpoint'),np.quantile(scores,.50,interpolation='midpoint'),np.quantile(scores,.75,interpolation='midpoint')

print("Average of quality score:",np.mean(scores))

print("Q1: {0}\nQ2: {1}\nQ3: {2}".format(Q1,Q2,Q3))
print("Total images",len(scores))

write_file(filename+"_best_images.txt",["{0} {1}".format(scores[i],names[i]) for i in range(len(scores)) if scores[i] <= Q1])
write_file(filename+"_worst_images.txt",["{0} {1}".format(scores[i],names[i]) for i in range(len(scores)) if scores[i] > Q3])
